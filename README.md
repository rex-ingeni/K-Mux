...
## TUI ↔ GUI — 같은 세션, 두 개의 얼굴

K-Mux의 핵심은 **GUI가 TUI를 대체하지 않고 그 위에 얹힌다**는 점입니다. 세션은 항상 진짜
CLI(실 PTY) 위에서 돌고, GUI는 그 스트림을 **파싱해서 다시 그린 화면**일 뿐입니다.

- 그래서 **터미널에서 쓰던 그대로** — 슬래시 커맨드, 권한 모드, 모델 선택, `/compact` 같은
  기능이 GUI에서도 그대로 동작합니다. GUI가 따로 흉내 낸 게 아니라 **같은 CLI**를 조종합니다.
- 세션 탭 하나에서 **터미널 탭 ↔ GUI 탭**을 언제든 오갈 수 있습니다. TUI에서 하던 대화를
  GUI로 넘겨 이어 보고, 다시 TUI로 돌아가도 **하나의 세션**입니다.
- 앱을 껐다 켜도 대화가 **복원**되어 다음 메시지부터 이어집니다.
