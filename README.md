...
<p align="center">
  <a href="https://rex-ingeni.github.io/K-Mux/"><b>🌐 소개 페이지 →</b></a>
</p>
...
<p align="center">
  <sub>한 화면에서 여러 세션을 분할 타일로 — 왼쪽은 GUI 채팅, 오른쪽은 Claude·Codex 터미널과 셸이 나란히.<br/>
  타일 경계를 드래그해 크기 조절, 상단바를 드래그해 위치 교체·분할.</sub>
</p>

## TUI ↔ GUI — 터미널과 GUI, 나란히

K-Mux의 핵심은 **GUI가 터미널을 대체하지 않는다**는 점입니다. 터미널 탭은 진짜 CLI를 그대로
돌리고, GUI 탭은 그 CLI가 말하는 **같은 스트림 프로토콜을 파싱해 그린 채팅 화면**입니다.
...
<p align="center">
  ...
  오른쪽 실제 Claude·Codex 터미널이 같은 화면에서 나란히.</sub>
</p>
...
- 🎨 **14가지 테마 색상** — 주황·파랑·초록·보라 등 원하는 강조색을 스와치로 선택. GUI와
  터미널 커서 색까지 함께 바뀝니다.
- 🔎 **글씨 크기 조절** — `⌘+ / ⌘− / ⌘0` 또는 트랙패드 **핀치 줌**으로 GUI·터미널 모두 확대/축소.
...
CLI의 입력줄을 그대로 쓰면서, macOS 텍스트 필드 같은 편집을 얹었습니다. 한글 입력(IME)에서도
안전하게 동작하고, Claude(테두리 있는 입력창)와 Codex(테두리 없는 입력창)를 자동으로 인식합니다.

- 🖱️ **마우스로 선택·이동** — 클릭해서 커서 이동, 드래그로 블록 선택 후 잘라내기/삭제.
- ⏭️ **단어·줄 단위 이동** — `⌥←/→`로 단어 단위, `⌘←/→`로 줄 처음·끝으로.
- ↩️ **실행 취소 · 다시 실행** — `⌘Z` / `⇧⌘Z`, 히스토리 방식이라 깜빡임 없이.
- ✍️ **덮어쓰기 입력·붙여넣기** — 블록을 선택한 채 입력하거나 붙여넣으면 그 자리에 교체됩니다.
- 🔲 **전체 선택 · 잘라내기** — `⌘A` / `⌘X`, 여러 줄 입력도 정확하게.

## 주요 기능

- 💬 **실시간 스트리밍 채팅** — 응답이 실시간으로 이어져 도착합니다.
- 📝 **마크다운 · 링크 미리보기 · 코드 하이라이트** — 인라인 SVG 다이어그램도 바로 렌더링.
- 🧰 **툴 카드** — 셸 실행 · 파일 편집 · MCP 툴 호출을 접이식 카드로 정리.
- 📁 **작업 폴더 패널** — 생성·수정된 파일을 한눈에.
- 📊 **컨텍스트 게이지 · 압축 표시** — 남은 컨텍스트를 한눈에, 압축되면 그 결과까지.
- 🪟 **분할 타일 · 그룹** — 여러 세션을 나란히 배치하고 그룹으로 묶어 관리.
- 🔒 **승인 카드 · 권한 모드** — 권한이 필요한 작업을 허용/거부, 수동·자동 등 모드 선택.
...
> *(함께 제공되는 `.app.zip`은 앱 **업데이트**가 사용하는 파일입니다 — 직접 받을 필요 없습니다.)*
